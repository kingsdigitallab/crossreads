...
export class AnyFileSystem {  
  ...
  */
  ...
  constructor(githubRepoPath=GITHUB_REPO_PATH) {
    ...
    this.isReadOnlyAndLocal = !githubRepoPath
  ...
  async writeJson(relativePath, data, sha=null, system=null) {
    ...
    if (DEBUG_DONT_SAVE || this.isReadOnlyAndLocal) {
      return {
        ok: false,
  ...
  guessSystemFromPath(path) {
    ...
    let canFetchLocally = ((this.isReadOnlyAndLocal && IS_BROWSER) || IS_BROWSER_LOCAL) && path.startsWith(this.prefixes[this.SYSTEMS.HTTP])

    if ((DEBUG_DONT_SAVE || this.isReadOnlyAndLocal) && canFetchLocally) {
